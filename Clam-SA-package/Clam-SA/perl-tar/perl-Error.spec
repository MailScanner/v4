...
find $RPM_BUILD_ROOT/usr -type f -print | \
	...
	grep -v "\.packlist" > Error-%{version}-filelist
if [ "$(cat Error-%{version}-filelist)X" = "X" ] ; then
    echo "ERROR: EMPTY FILE LIST"
    exit 1
fi

%files -f Error-%{version}-filelist
%defattr(-,root,root)
